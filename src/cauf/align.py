...
Two assumptions, both stated so they can be attacked:

  1. A mark inside an edited span is erased.  In reality an invisible character
     immediately before an edited word often survives, because the editor
     selected the word and not the character before it.  Treating it as lost
     *under-counts misaddressing*, which is the dangerous outcome, so this
     assumption makes the results optimistic.  The synthetic ``reword`` channel
     exists to cover the case honestly, and the two should be read together.

  2. Token-level diffing approximates what a text editor does.  It does not
     model cut-and-paste as a move; a moved paragraph reads as delete plus
     insert, so paragraph reordering looks worse here than it is.  Use the
     synthetic ``reorder_paragraphs`` channel for that case.

The point of the real-pair path is not to replace the synthetic channels.  It
is to check that the synthetic edit *magnitudes* are in the right range, so the
headline numbers are not an artefact of parameters we chose ourselves.
"""
...
def gap_map(src: Sequence[str], dst: Sequence[str]) -> Dict[int, int]:
    """Map gap indices in ``src`` to gap indices in ``dst``.

    Gap ``g`` sits before token ``g``.  A gap survives when it falls at or
    inside a matching block; the image is the corresponding offset in ``dst``.
    Gaps inside changed regions are absent from the result.
    """
    sm = SequenceMatcher(a=list(src), b=list(dst), autojunk=False)
    out: Dict[int, int] = {}
    for a, b, size in sm.get_matching_blocks():
        for k in range(size + 1):          # +1: the gap after the last token
            g = a + k
            if g not in out:
                out[g] = b + k
    return out
...
def measure_pair(pair: Pair, key: bytes, spec: AnchorSpec) -> Tally:
    """Classify every anchor of ``before`` against its image in ``after``."""
...
def edit_magnitude(pair: Pair) -> float:
    """Fraction of tokens that changed, for binning pairs by edit size."""
...
def bin_pairs(pairs: Sequence[Pair],
    ...
    """Group revision pairs by how much actually changed.

    Reporting a single average over Wikipedia revisions is close to
    meaningless: most edits are typo fixes and a few are rewrites.  The model
    needs ``q`` as a function of edit magnitude, not a mean over a mixture.
    """
